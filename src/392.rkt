...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 392ex) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; Exercise 390
;; ------------
...
; traverses the tree using lodir to produce the leaf symbol
...
(check-expect (tree-pick.v2 t5 '(left left left)) 'a)
(check-expect (tree-pick.v2 t5 '(left right right)) 'o)
(check-error (tree-pick.v2 t5 '(left left)) "tree or lodir were too too small")
(check-error (tree-pick.v2 t5 '(left right right 'left)) "tree or lodir were too too small")


(define (tree-pick.v2 tree lodir)
  (cond
    [(and (empty? lodir) (symbol? tree)) tree]
    [(and (cons? lodir) (branch? tree))
     (tree-pick.v2 (if (equal? 'left (first lodir))
                       (branch-left tree)
                       (branch-right tree))
                   (rest lodir))]
    [else (error "tree or lodir were too too small")]))

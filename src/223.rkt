...
; moves the block (if possible) using "left" or "right"
(define (key-h t k)
  (cond
    [(key=? "left" k) (if (on-zero-col? t) t (move-block-left t))]
    [(key=? "right" k) (if (on-zero-col? t) t (move-block-right t))]
    ;[(key=? " " k) (put-block-down t)]
    [else t]))


; Tetris -> Boolean
(define (on-zero-col? t)
  (on-zero-block? (tetris-block t)))

; Landscape -> Boolean
(define (on-zero-block? b)
  (= 0 (block-y b)))


(check-expect (key-h (make-tetris (make-block 4 4) '()) "left") (make-tetris (make-block 3 4) '()))
(check-expect (key-h (make-tetris (make-block 4 0) '()) "left") (make-tetris (make-block 4 0) '()))
(check-expect (key-h (make-tetris (make-block 4 4) '()) "right") (make-tetris (make-block 5 4) '()))
(check-expect (key-h (make-tetris (make-block 4 0) '()) "right") (make-tetris (make-block 4 0) '()))
(check-expect (key-h (make-tetris (make-block 4 4) '()) "w") (make-tetris (make-block 4 4) '()))
(check-expect (on-zero-col? (make-tetris (make-block 4 0) '())) #true)
(check-expect (on-zero-col? (make-tetris (make-block 0 4) '())) #false)


; ------------------ STOP-WHEN  -------------------

; Tetris -> Boolean
; is any block in the landscape too tall?
(define (end? t)
  (any-landscape-col-zero? (tetris-landscape t)))

; Landscape -> Boolean
; has any block in the landscape reached 0
(define (any-landscape-col-zero? l)
  (cond
    [(empty? l) #false]
    [else (or (on-zero-block? (first l))
              (any-landscape-col-zero? (rest l)))]))


(check-expect (end? (make-tetris (make-block 4 4) (cons (make-block 4 0) landscape1))) #true)
(check-expect (end? tetris1-drop) #false)

(check-expect (any-landscape-col-zero? landscape1) #false)
(check-expect (any-landscape-col-zero? (cons (make-block 4 0) landscape1)) #true)

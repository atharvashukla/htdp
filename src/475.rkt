...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 475ex) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; Exercise 475
;; ------------
...
(define (find-path orig dest G)
  (local (; [List-of Node] Node Graph -> [Maybe Path]
          ...
          (define (find-path/list lo-Os D)
            (foldr (λ (e a)
                     (local ((define candidate (find-path-h e D)))
                       (if  (list? candidate)
                            candidate
                            a)))
                   #false
...
; Ormap is not helpful, it returns a boolean. We need to return the first
; application of find-path-h that is not a boolean. foldr proved useful
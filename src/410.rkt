...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 410ex) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;; Exercise 410
;; ------------
...
; DB DB -> DB
; merges the content of db1 and db1
; ASSUMPTION: they have the same schema

(check-expect (db-content  (db-union school-db school-db))
              (append school-content))

(check-expect (db-content (db-union school-db school-db-2))
              (append school-content (rest school-content-2)))

(define (db-union db1 db2)
  (local ((define db1-labels (map spec-label (db-schema db1)))
          (define new-content (remove-duplicates
                               (append (db-content db1)
                                       (db-content (reorder db2 db1-labels))))))
    (make-db (db-schema db1) content-no-dup)))
